...
ISA_FILE="${ISA_FILE:-$ROOT_DIR/QKV_DSE.py}"
...
if [[ "$SKIP_REGEN" != "1" ]]; then
  if ! "$PYTHON_BIN" -c "import antlr4" >/dev/null 2>&1; then
    ...
    echo "Run this script inside the ACT container/environment where QKV_DSE.py already works." >&2
    exit 1
  fi
  echo "[1/2] Regenerating QKV_DSE backend artifacts from $ISA_FILE"
...
if [[ ! -f "$PRIMITIVE_JSON" ]]; then
  ...
  echo "Run without SKIP_REGEN=1, or verify QKV_DSE backend generation succeeded." >&2
  exit 1
fi
